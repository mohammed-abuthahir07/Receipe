...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($title === '' || $description === '' || !$cuisineId || $hero === '') {
        ...
        $error = 'Before publishing, enter nutrition (calories required, macros cannot be negative).';
        ...
        try {
            ...
            if ($action === 'publish') {
                ...
                $pdo->commit();
                flash('success', 'Recipe completed and published with video + nutrition.');
    ...
    <div class="container">
        <div class="section__head">
            ...
            <p>
                ...
                Admin must fill ingredients, full preparation, nutrition and short video before it goes live.
            </p>
        </div>

        <?php if ($error): ?>
            <div class="panel" style="border-color:#f3b0a8;margin-bottom:1rem;color:var(--danger)"><?= e($error) ?></div>
        <?php endif; ?>
        ...
        <form method="post" class="panel admin-form" data-reveal data-admin-recipe>
            ...
            <h3>Basics</h3>
            <div class="form-field"><label>Title</label><input name="title" value="<?= e($recipe['title']) ?>" required></div>
            <div class="form-field"><label>Description</label><textarea name="description" rows="3" required><?= e($recipe['description']) ?></textarea></div>
            <div class="form-grid">
                ...
                <div class="form-field"><label>Prep mins</label><input type="number" name="prep_time_mins" value="<?= (int) $recipe['prep_time_mins'] ?>" min="1"></div>
                <div class="form-field"><label>Cook mins</label><input type="number" name="cook_time_mins" value="<?= (int) $recipe['cook_time_mins'] ?>" min="1"></div>
                <div class="form-field"><label>Servings (baseline)</label><input type="number" name="servings" value="<?= (int) $recipe['servings'] ?>" min="1"></div>
            </div>
            <div class="form-field"><label>Hero image URL</label><input name="hero_image_url" value="<?= e($recipe['hero_image_url']) ?>" required></div>
            <div class="form-field">
                <label>Short cooking video URL (mp4) — required to publish</label>
                <input name="video_clip_url" value="<?= e((string) ($recipe['video_clip_url'] ?? '')) ?>" placeholder="https://...mp4">
                <small style="color:var(--muted)">Shown as a small trailer on the recipe page (autoplay muted).</small>
            </div>
            ...
            <div data-ing-list>
                <?php foreach ($ingredients as $ing): ?>
                    <div class="repeater-row" data-ing-row>
                        <input name="ing_name[]" placeholder="Ingredient" value="<?= e($ing['name']) ?>">
                        <input name="ing_qty[]" type="number" step="0.01" placeholder="Qty" value="<?= e(format_qty($ing['quantity'] ?? 1)) ?>">
            ...
            <button type="button" class="btn btn--ghost" data-add-ing style="margin:.5rem 0 1.25rem">+ Add ingredient</button>
            ...
            <div data-step-list>
                <?php foreach ($steps as $step): ?>
                    <div class="repeater-row repeater-row--step" data-step-row>
                        ...
                        <input name="step_timer[]" type="number" min="0" placeholder="Timer sec (optional)" value="<?= e((string) ($step['timer_seconds'] ?? '')) ?>">
                        <button type="button" class="btn btn--ghost" data-remove-row>Remove</button>
                    </div>
                <?php endforeach; ?>
            </div>
            <button type="button" class="btn btn--ghost" data-add-step style="margin:.5rem 0 1.25rem">+ Add step</button>

            <h3>Nutrition <small style="color:var(--muted);font-weight:400">(per serving)</small></h3>
            <div class="form-grid">
                ...
                <div class="form-field"><label>Fiber (g)</label><input type="number" step="0.01" name="fiber_g" value="<?= e((string) ($nutrition['fiber_g'] ?? '')) ?>"></div>
