(function () {
  ...
  form.querySelector('[data-add-ing]')?.addEventListener('click', () => {
    ...
    row.dataset.ingRow = '';
    row.innerHTML = `
      <input name="ing_name[]" placeholder="Ingredient name">
      <input name="ing_qty[]" type="number" step="0.01" placeholder="Quantity" value="1">
  ...
  form.querySelector('[data-add-step]')?.addEventListener('click', () => {
    ...
    row.innerHTML = `
      ...
      <input name="step_timer[]" type="number" min="0" placeholder="Timer (seconds, optional)">
      <button type="button" class="btn btn--ghost" data-remove-row>Remove</button>`;
